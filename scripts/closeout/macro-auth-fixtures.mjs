...
const envPath = "/tmp/macro-supabase.env";
...
const sql = (text) => execFileSync("psql", [dbUrl,"--no-psqlrc","-X","-v","ON_ERROR_STOP=1","-q"], { input:text, stdio:["pipe","inherit","inherit"] });
async function admin(path, init={}) {
  const response=await fetch(`${apiUrl}/auth/v1/admin${path}`, { ...init, headers:{apikey:serviceKey,Authorization:`Bearer ${serviceKey}`,"Content-Type":"application/json",...(init.headers||{})} });
...
if(action==="cleanup") {
  ...
  const ids=Object.values(fixture.accounts).map((a)=>a.id);
  ...
  const remaining=execFileSync("psql",[dbUrl,"-Atqc",`select count(*) from auth.users where email like '${prefix}-%@example.test'`],{encoding:"utf8"}).trim();
...
sql(`
begin;
insert into public.tenants(id,slug,name,status,is_primary,default_locale,supported_locales) values
 ('${tenants.a}','${prefix}-tenant-a','Closeout Tenant A','active',false,'ar',array['ar','en','tr']),
...
insert into public.tenant_domains(tenant_id,hostname,status,is_primary,verified_at) values
 ('${tenants.a}','127.0.0.1','active',true,now()),('${tenants.b}','tenant-b.closeout.test','active',true,now())
on conflict(hostname) do update set tenant_id=excluded.tenant_id,status='active',verified_at=now();
${Object.values(created).map((a)=>`insert into public.tenant_memberships(tenant_id,user_id,role,status,permissions,mfa_required) values ('${tenants[a.tenant]}','${a.id}','${a.role}','active','{}',false) on conflict(tenant_id,user_id) do update set role=excluded.role,status='active';`).join("\n")}
insert into public.portal_profiles(user_id,display_name,locale,status,marketing_opt_in,ai_opt_out)
select user_id,'Closeout '||role,'ar','active',false,false from public.tenant_memberships where tenant_id in ('${tenants.a}','${tenants.b}')
on conflict(user_id) do update set status='active';
commit;`);
const fixture={prefix,apiUrl,tenants,accounts:created};
await writeFile(fixturePath,JSON.stringify(fixture),{mode:0o600}); await chmod(fixturePath,0o600);
