...
function psql(sql) {
  execFileSync("psql", [dbUrl, "--no-psqlrc", "-X", "-v", "ON_ERROR_STOP=1", "-q"], { input: sql, stdio: ["pipe", "inherit", "inherit"] });
}
function scalar(sql) {
...
on conflict (id) do update set status='published',price_amount=excluded.price_amount,availability=excluded.availability;
...
`);
fixture.macro = ids;
await import("node:fs/promises").then(({ writeFile }) => writeFile(fixturePath, JSON.stringify(fixture), { mode: 0o600 }));
