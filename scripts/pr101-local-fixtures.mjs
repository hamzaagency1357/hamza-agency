...
async function adminCreateUser(account) {
  ...
  if (!response.ok) throw new Error(`local_auth_user_create_failed:${response.status}`);
  const body = await response.json();
  if (!body?.id) throw new Error("local_auth_user_id_missing");
  return body.id;
}
...
async function passwordGrant(account) {
  ...
  if (!response.ok) throw new Error(`local_auth_login_failed:${response.status}`);
  const body = await response.json();
  if (!body?.access_token) throw new Error("local_access_token_missing");
...
values
  (${sqlLiteral(tenants.a)}::uuid,'pr101-tenant-a','PR101 Tenant A','active',false,'ar',array['ar','en','tr']),
  (${sqlLiteral(tenants.b)}::uuid,'pr101-tenant-b','PR101 Tenant B','active',false,'ar',array['ar','en','tr'])
...
values
  (${sqlLiteral(tenants.a)}::uuid,'tenant-a.test','active',true,now()),
  (${sqlLiteral(tenants.b)}::uuid,'tenant-b.test','active',true,now())
...
values
  (${sqlLiteral(tenants.a)}::uuid,${sqlLiteral(adminId)}::uuid,'super_admin','active','{}'::jsonb,true),
  (${sqlLiteral(tenants.b)}::uuid,${sqlLiteral(invitedId)}::uuid,'creator','active','{}'::jsonb,false)
on conflict (tenant_id,user_id) do update set role=excluded.role,status='active',permissions=excluded.permissions,mfa_required=excluded.mfa_required,updated_at=now();
`;
