...
const users = {
  ...
  expired: { email: "pr101-expired@example.test", password: "LocalOnly-Expired-2026!" },
};
...
const ids = {
  admin: await adminCreateUser(users.admin),
  invited: await adminCreateUser(users.invited),
  expired: await adminCreateUser(users.expired),
};
...
const fixture = {
  apiUrl, dbUrl, serviceRoleKey, tenants, hosts,
  users: {
    admin: { ...users.admin, id: ids.admin, accessToken: await passwordGrant(users.admin) },
    invited: { ...users.invited, id: ids.invited, accessToken: await passwordGrant(users.invited) },
    expired: { ...users.expired, id: ids.expired, accessToken: await passwordGrant(users.expired) },
  },
};
...
console.log(JSON.stringify({ ok: true, synthetic_users: 3, synthetic_tenants: 2 }));
