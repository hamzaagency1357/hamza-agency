...
export default function ProgramsPageContent({
  ...
  const highlights = [
    copy("programsHighlightDirectApply"),
    copy("programsHighlightReview"),
    copy("programsHighlightWhatsApp"),
    copy("programsHighlightBestFit"),
  ];

  return (
    <section
      dir={getLanguageDirection(language)}
      className="relative z-10 mx-auto max-w-7xl px-5 py-16"
    >
      ...
      <div className="mt-14 rounded-[2rem] border border-yellow-400/20 bg-yellow-500/10 p-7 text-center backdrop-blur">
        <h2 className="text-2xl font-black text-yellow-100">{copy("programsHelpTitle")}</h2>

        <p className="mx-auto mt-3 max-w-2xl leading-8 text-white/70">
          {copy("programsHelpDescription")}
        </p>
