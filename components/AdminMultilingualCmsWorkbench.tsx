...
export default function AdminMultilingualCmsWorkbench(){
 ...
 async function load(){const client=supabase;if(!client)return;setLoading(true);setMessage("");
  const entityResults=await Promise.all(configs.map((config)=>client.from(config.table).select("*").limit(500)));
  const settingsResult=await client.from("settings").select("*").in("setting_key",settingKeys).limit(100);
  const loaded:Item[]=[];
  ...
  const translations=await client.from("content_translations").select("source_type,source_id,field_name,language,translated_value,status,reviewed,is_published").in("language",["ar","en","tr"]).limit(10000);
 ...
 async function save(publish=false){const client=supabase;if(!client||!item)return;if(publish&&!complete(language)){setMessage("لا يمكن نشر ترجمة ناقصة.");return}setSaving(true);setMessage("");const rows=(Object.keys(fieldLabels) as FieldKey[]).map((field)=>({source_type:item.sourceType,source_id:item.sourceId,field_name:field,language,translated_value:pack[item.key]?.[language]?.[field]?.translated_value.trim()||"",status:publish?"published":"draft",reviewed:publish,is_published:publish,updated_at:new Date().toISOString()}));const result=await adminBoundaryMutation("pr116_admin_cms_translation_upsert",{values:rows,filters:[],select:undefined,returnMode:"many",options:{onConflict:"source_type,source_id,field_name,language"}});setSaving(false);if(result.error){setMessage("تعذر حفظ الترجمة. راجع المحتوى والصلاحيات ثم حاول مجددًا.");return}setMessage(publish?"تم نشر الترجمة المكتملة.":"تم حفظ المسودة.");await load()}
 if(!authorized||loading)return <main dir="rtl" className="min-h-screen bg-[#070009] p-8 text-white">جارٍ تحميل مركز الترجمة...</main>;
 return <main dir="rtl" className="min-h-screen overflow-x-hidden bg-[#070009] p-4 text-white md:p-8"><div className="mx-auto max-w-7xl"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-sm font-bold text-purple-300">إدارة المحتوى بالعربية والإنجليزية والتركية</p><h1 className="mt-2 text-4xl font-black">مركز المحتوى متعدد اللغات</h1></div><Link href="/admin" className="rounded-full border border-white/10 px-5 py-3">لوحة الإدارة</Link></div>
