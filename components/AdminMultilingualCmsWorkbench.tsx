...
export default function AdminMultilingualCmsWorkbench(){
 ...
 async function save(publish=false){const client=supabase;if(!client||!item)return;if(publish&&!complete(language)){setMessage("لا يمكن نشر ترجمة ناقصة.");return}setSaving(true);setMessage("");const rows=(Object.keys(fieldLabels) as FieldKey[]).map((field)=>({source_type:item.sourceType,source_id:item.sourceId,field_name:field,language,translated_value:pack[item.key]?.[language]?.[field]?.translated_value.trim()||"",status:publish?"published":"draft",reviewed:publish,is_published:publish,updated_at:new Date().toISOString()}));const result=await client.from("content_translations").upsert(rows,{onConflict:"source_type,source_id,field_name,language"});setSaving(false);if(result.error){setMessage(`تعذر الحفظ: ${result.error.message}`);return}setMessage(publish?"تم نشر الترجمة المكتملة.":"تم حفظ المسودة.");await load()}
