...
export default function TenantGovernanceConsole() {
  ...
  async function audit(action: string, entityType: string, entityId: string | null, afterData: Row) { if (supabase && tenant && actorId) await supabase.from("tenant_admin_audit").insert({ tenant_id: tenant.id, actor_id: actorId, action, entity_type: entityType, entity_id: entityId, after_data: afterData }); }
  async function saveTenant() { if (!supabase || !tenant) return; const result = await supabase.from("tenants").update({ name: tenant.name.trim(), default_locale: tenant.default_locale, supported_locales: tenant.supported_locales, updated_at: new Date().toISOString() }).eq("id", tenant.id); if (result.error) return setMessage(result.error.message); await audit("tenant.updated", "tenant", tenant.id, { name: tenant.name }); setMessage("تم حفظ إعدادات المستأجر."); }
  async function saveBranding() { if (!supabase || !tenant) return; const payload = { ...branding, tenant_id: tenant.id, social_links: safeJson(socialJson), legal_overrides: safeJson(legalJson), updated_at: new Date().toISOString() }; const result = await supabase.from("tenant_branding").upsert(payload); if (result.error) return setMessage(result.error.message); await audit("tenant.branding_updated", "tenant_branding", tenant.id, { primary_color: payload.primary_color }); setMessage("تم حفظ الهوية."); }
  async function addDomain() { if (!supabase || !tenant) return; const hostname = cleanHostname(domainDraft); if (!validHostname(hostname)) return setMessage("اسم النطاق غير صالح."); const result = await supabase.from("tenant_domains").insert({ tenant_id: tenant.id, hostname, status: "pending", is_primary: domains.length === 0 }); if (result.error) return setMessage(result.error.message); await audit("tenant.domain_added", "tenant_domain", hostname, { hostname }); setDomainDraft(""); await load(); }
  async function toggleFlag(flag: Flag) { if (!supabase || !tenant) return; const result = await supabase.from("tenant_feature_flags").update({ enabled: !flag.enabled, updated_at: new Date().toISOString() }).eq("tenant_id", tenant.id).eq("feature_key", flag.feature_key); if (result.error) return setMessage(result.error.message); await load(); }
  async function saveSetting() { if (!supabase || !tenant || !settingDraft.key.trim()) return; const key = settingDraft.key.trim().toLowerCase().replace(/[^a-z0-9_.-]/g, "_").slice(0, 100); let value: unknown = settingDraft.value; try { value = JSON.parse(settingDraft.value); } catch {} const result = await supabase.from("tenant_settings").upsert({ tenant_id: tenant.id, key, value, is_secret: false, updated_at: new Date().toISOString() }); if (result.error) return setMessage(result.error.message); await audit("tenant.setting_updated", "tenant_setting", key, { key }); setSettingDraft({ key: "", value: "" }); await load(); }
  ...
  return <main className="min-h-screen bg-[#09050f] px-4 py-10 text-white" dir="rtl"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="rounded-3xl border border-white/10 bg-white/5 p-6"><h2 className="text-2xl font-bold">Feature Flags</h2><div className="mt-3 grid gap-2 md:grid-cols-3">{flags.map((flag) => <button key={flag.feature_key} onClick={() => void toggleFlag(flag)} className="rounded-xl border border-white/10 p-3 text-right">{flag.feature_key} · {flag.enabled ? "enabled" : "disabled"}</button>)}</div></section>
