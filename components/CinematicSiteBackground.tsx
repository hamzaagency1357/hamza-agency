...
export default function CinematicSiteBackground() {
  ...
  const selected = media
    ? selectDeviceVisualSources(media, mobile)
    : { primary: null, fallback: null };
  const canPlay =
    media?.fileType === "cinematic_video" &&
    !videoFailed && Boolean(selected.primary) &&
  ...
  return (
    <div className="hamza-cinematic-site-background" aria-hidden="true" data-scope={scope} data-mode={canPlay ? "video" : staticAsset ? "poster" : "fallback"}>
      ...
      {staticAsset && (
        <div className="hamza-cinematic-poster" style={{
          backgroundImage: backgroundImage(staticAsset),
          backgroundPosition: media?.focalPosition || "center center",
          filter: media?.blurPx ? `blur(${media.blurPx}px)` : undefined,
      ...
      {canPlay && media && selected.primary && (
        <video
          ...
          muted autoPlay loop={media.loop} playsInline preload="metadata"
      ...
      <div className="hamza-cinematic-dim" style={{ opacity: media?.dimming ?? 0.34 }} />
      <div className="hamza-cinematic-overlay" style={{ opacity: media?.overlayStrength ?? 0.42 }} />
