...
const copy = {
  ...
  en: {
    ...
    managementBody: "HAMZA AGENCY is managed by عراب سوريا, following professional standards focused on trust, privacy, and careful follow-up.",
  ...
  tr: {
    ...
    managementBody: "HAMZA AGENCY, güven, gizlilik ve titiz takibe odaklanan profesyonel standartlar doğrultusunda عراب سوريا yönetiminde çalışır.",
...
function useAboutCopy() { const language = useSiteLanguage(); return { language, text: copy[language], direction: getLanguageDirection(language) }; }
...
export function AboutBackLink() { const { language, text } = useAboutCopy(); return <Link href={localizePublicHref("/", language)} className="mb-8 inline-block text-purple-200">{text.back}</Link>; }
...
export function AboutCta({ cleanWhatsapp }: { cleanWhatsapp: string }) { const { language, text } = useAboutCopy(); return <div className="mt-10 rounded-[2rem] border border-green-400/20 bg-green-500/10 p-7 text-center backdrop-blur"><h2 className="text-3xl font-black">{text.ctaTitle}</h2><p className="mx-auto mt-4 max-w-2xl leading-8 text-white/70">{text.ctaText}</p><div className="mt-7 flex flex-col justify-center gap-4 sm:flex-row"><Link href={localizePublicHref("/programs", language)} className="rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 px-7 py-4 font-black">{text.programs}</Link><a href={`https://wa.me/${cleanWhatsapp}`} target="_blank" rel="noreferrer" className="rounded-full bg-green-500 px-7 py-4 font-black text-white">{text.whatsapp}</a></div></div>; }
