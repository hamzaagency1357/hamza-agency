...
export default function PortalAccountModule({ role, moduleKey }: { role: PortalRole; moduleKey: "profile" | "privacy" | "sessions" | "notifications" }) {
  ...
  async function createPrivacyRequest(type: typeof requestTypes[number]) {
    if (!supabase || !membership || !userId) return;
    setMessage("");
    const result = await supabase.from("privacy_requests").insert({ tenant_id: membership.tenant_id, user_id: userId, request_type: type, status: "submitted", details: {}, due_at: new Date(Date.now() + 30 * 86_400_000).toISOString() });
  ...
  async function togglePreference(channel: typeof channels[number], eventKey: typeof events[number]) {
    ...
    const result = await supabase.from("portal_notification_preferences").upsert({ tenant_id: membership.tenant_id, user_id: userId, channel, event_key: eventKey, enabled: !current, updated_at: new Date().toISOString() });
  ...
  return (
    <main className="min-h-screen bg-[#09050f] px-4 py-24 text-white" dir="rtl">
      <section className="mx-auto max-w-5xl rounded-3xl border border-violet-300/20 bg-white/5 p-6 shadow-2xl">
        ...
        {moduleKey === "profile" && profile && <div className="mt-6 grid gap-4 sm:grid-cols-2"><label>الاسم المعروض<input value={profile.display_name ?? ""} onChange={(event) => setProfile({ ...profile, display_name: event.target.value })} className="mt-1 min-h-12 w-full rounded-xl border border-white/10 bg-black/40 px-4" /></label><label>رقم التواصل<input value={profile.phone ?? ""} onChange={(event) => setProfile({ ...profile, phone: event.target.value })} className="mt-1 min-h-12 w-full rounded-xl border border-white/10 bg-black/40 px-4" /></label><label>اللغة<select value={profile.locale} onChange={(event) => setProfile({ ...profile, locale: event.target.value as Profile["locale"] })} className="mt-1 min-h-12 w-full rounded-xl border border-white/10 bg-black/70 px-4"><option value="ar">العربية</option><option value="en">English</option><option value="tr">Türkçe</option></select></label><label className="flex min-h-12 items-center justify-between rounded-xl border border-white/10 px-4">عدم استخدام الذكاء الاصطناعي<input type="checkbox" checked={profile.ai_opt_out} onChange={(event) => setProfile({ ...profile, ai_opt_out: event.target.checked })} /></label><label className="flex min-h-12 items-center justify-between rounded-xl border border-white/10 px-4">رسائل تسويقية<input type="checkbox" checked={profile.marketing_opt_in} onChange={(event) => setProfile({ ...profile, marketing_opt_in: event.target.checked })} /></label><button type="button" onClick={() => void saveProfile()} className="min-h-12 rounded-xl bg-violet-600 px-5 font-bold">حفظ الملف</button></div>}
        {moduleKey === "privacy" && <div className="mt-6 space-y-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{requestTypes.map((type) => <button key={type} type="button" onClick={() => void createPrivacyRequest(type)} className="min-h-12 rounded-xl border border-white/10 bg-black/30 px-4">{type}</button>)}</div><div className="space-y-3">{requests.map((request) => <div key={request.id} className="rounded-xl border border-white/10 p-4"><strong>{request.request_type}</strong><span className="mx-3 text-white/50">{request.status}</span><time className="text-sm text-white/50">{new Date(request.created_at).toLocaleString("ar")}</time></div>)}</div></div>}
