...
const identity = {
  ar: {
    managedBy: "بإدارة الوكيل ⚔عܓོراب✴سܓོوريا⚔",
    semanticIdentity: "HAMZA AGENCY بإدارة الوكيل عراب سوريا",
    ...
    >
      <div className="mx-auto flex max-w-7xl flex-col gap-3">
        <div className="flex items-center justify-between gap-3">
          ...
          >
            <Image
              ...
              height={48}
              priority
              className="h-12 w-12 shrink-0 rounded-2xl border border-yellow-300/25 object-cover"
            />
            <span className="min-w-0">
              ...
              <span className="sr-only">{t.semanticIdentity}</span>
            </span>
          </Link>
          <LanguageSwitcher />
        </div>
        ...
        <nav
          ...
          className="overflow-x-auto rounded-2xl border border-white/10 bg-white/[0.035] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
          data-testid="public-primary-navigation"
        >
          <div className="flex min-w-max items-center gap-1 p-1 sm:min-w-full sm:justify-between">
            {navigation[language].map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`inline-flex min-h-11 shrink-0 items-center justify-center whitespace-nowrap rounded-xl px-3 py-2 text-center text-xs font-black transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-300 sm:px-4 sm:text-sm ${
                    active
                      ? "text-yellow-200"
