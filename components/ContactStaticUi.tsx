...
const copy: Record<"ar" | "en" | "tr", Copy> = {
  ...
  en: {
    ...
    quickLinks: [
      { title: "Programs", href: "/programs", text: "Browse currently available programs and start a joining request." },
      { title: "Agency services", href: "/services", text: "Learn about management, support, and follow-up services for content creators." },
      { title: "Digital services", href: "/digital-services", text: "Learn about top-up, withdrawal, and WhatsApp follow-up services." },
      { title: "Track application", href: "/application-status", text: "Track your joining application using the available request details." },
    ],
  ...
  tr: {
    ...
    quickLinks: [
      { title: "Programlar", href: "/programs", text: "Mevcut programları inceleyin ve katılım başvurusu başlatın." },
      { title: "Ajans hizmetleri", href: "/services", text: "İçerik üreticileri için yönetim, destek ve takip hizmetleri hakkında bilgi alın." },
      { title: "Dijital hizmetler", href: "/digital-services", text: "Yükleme, para çekme ve WhatsApp üzerinden takip hizmetleri hakkında bilgi alın." },
      { title: "Başvuruyu takip et", href: "/application-status", text: "Mevcut başvuru bilgilerini kullanarak katılım başvurunuzu takip edin." },
    ],
...
export function ContactNowLabel() {
  const { text } = useCopy();
  return <>{text.contactNow}</>;
}
...
export function ContactEmailAndHoursCards({ email, workingHours }: { email: string; workingHours: string }) {
  ...
  return (
    <>
      {email ? <div dir={getLanguageDirection(language)} className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-6 backdrop-blur"><h2 className="text-3xl font-black">{text.emailTitle}</h2><p className="mt-4 break-words text-xl font-bold text-green-200">{email}</p><p className="mt-4 leading-8 text-white/65">{text.emailNote}</p><a href={`mailto:${email}`} className="mt-6 inline-flex rounded-full bg-green-500 px-6 py-3 font-black text-white">{text.sendEmail}</a></div> : null}
      <div dir={getLanguageDirection(language)} className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-6 backdrop-blur"><h2 className="text-3xl font-black">{text.hoursTitle}</h2><p className="mt-4 break-words text-xl font-bold text-green-200">{workingHours}</p><p className="mt-4 leading-8 text-white/65">{text.hoursNote}</p><Link href="/programs" className="mt-6 inline-flex rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 px-6 py-3 font-black text-white">{text.viewPrograms}</Link></div>
    </>
  );
}
