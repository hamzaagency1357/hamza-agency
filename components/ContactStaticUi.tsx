...
export function ContactEmailAndHoursCards({ email, workingHours }: { email: string; workingHours: string }) {
  ...
  const displayWorkingHours = workingHours
    ? localizeDynamicPublicCopy(workingHours, language)
    : text.hoursFallback;
...
export function ContactSocialLinks({
  ...
  return (
    <div
      ...
      className="mt-10 rounded-[2rem] border border-cyan-400/20 bg-cyan-500/10 p-7 backdrop-blur"
    >
      <h2 className="text-3xl font-black">{text.socialTitle}</h2>
      <p className="mt-4 leading-8 text-white/65">{text.socialText}</p>
