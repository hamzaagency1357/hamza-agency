...
const copy: Record<"ar" | "en" | "tr", Copy> = {
  ar: {
    ...
    beforeText: "لتسريع الرد، أرسل اسمك، الدولة، البرنامج أو الخدمة المطلوبة، ورقم واتساب صحيح. وإذا كنت تتابع طلباً سابقاً، استخدم نفس رقم الواتساب الذي أرسلته في الطلب.",
    quickLinks: [{ title: "البرامج", href: "/programs", text: "تصفح البرامج المتاحة حالياً وابدأ طلب الانضمام." }, { title: "خدمات الوكالة", href: "/services", text: "تعرف على خدمات الإدارة والدعم والمتابعة لصناع المحتوى." }, { title: "الخدمات الرقمية", href: "/digital-services", text: "تعرف على خدمات الشحن والسحب والمتابعة عبر واتساب." }, { title: "تتبع الطلب", href: "/application-status", text: "تابع حالة طلب الانضمام باستخدام بيانات الطلب المتاحة." }],
    openWhatsApp: "فتح واتساب",
    whatsappMessage: "مرحباً، أريد التواصل مع وكالة حمزة.",
    socialTitle: "روابطنا الرسمية",
    socialText: "تظهر هنا فقط الروابط الرسمية المحفوظة في إعدادات الموقع.",
  },
  en: {
    ...
    beforeText: "To help us respond faster, send your name, country, requested program or service, and a valid WhatsApp number. When following up on a previous request, use the same WhatsApp number submitted with that request.",
    quickLinks: [{ title: "Programs", href: "/programs", text: "Browse currently available programs and start a joining request." }, { title: "Agency services", href: "/services", text: "Learn about management, support, and follow-up services for content creators." }, { title: "Digital services", href: "/digital-services", text: "Learn about top-up, withdrawal, and WhatsApp follow-up services." }, { title: "Track application", href: "/application-status", text: "Track your joining application using the available request details." }],
    openWhatsApp: "Open WhatsApp",
    whatsappMessage: "Hello, I would like to contact HAMZA AGENCY.",
    socialTitle: "Official links",
    socialText: "Only official links saved in the site settings appear here.",
  },
  tr: {
    ...
    beforeText: "Daha hızlı yanıt alabilmek için adınızı, ülkenizi, istediğiniz program veya hizmeti ve geçerli bir WhatsApp numarasını gönderin. Önceki bir başvuruyu takip ediyorsanız, başvuruda kullandığınız aynı WhatsApp numarasını kullanın.",
    quickLinks: [{ title: "Programlar", href: "/programs", text: "Mevcut programları inceleyin ve katılım başvurusu başlatın." }, { title: "Ajans hizmetleri", href: "/services", text: "İçerik üreticileri için yönetim, destek ve takip hizmetleri hakkında bilgi alın." }, { title: "Dijital hizmetler", href: "/digital-services", text: "Yükleme, para çekme ve WhatsApp üzerinden takip hizmetleri hakkında bilgi alın." }, { title: "Başvuruyu takip et", href: "/application-status", text: "Mevcut başvuru bilgilerini kullanarak katılım başvurunuzu takip edin." }],
    openWhatsApp: "WhatsApp'ı aç",
    whatsappMessage: "Merhaba, HAMZA AGENCY ile iletişime geçmek istiyorum.",
    socialTitle: "Resmî bağlantılarımız",
    socialText: "Burada yalnızca site ayarlarında kayıtlı resmî bağlantılar gösterilir.",
  },
};
...
export function ContactSocialLinks({ links }: { links: Array<{ label: string; href: string }> }) {
  const { language, text } = useCopy();
  if (!links.length) return null;
  return <div dir={getLanguageDirection(language)} className="mt-10 rounded-[2rem] border border-cyan-400/20 bg-cyan-500/10 p-7 backdrop-blur"><h2 className="text-3xl font-black">{text.socialTitle}</h2><p className="mt-4 leading-8 text-white/65">{text.socialText}</p><div className="mt-6 flex flex-wrap gap-3">{links.map((link) => <a key={link.label} href={link.href} target="_blank" rel="noreferrer" className="rounded-full border border-white/10 bg-white/[0.05] px-5 py-3 font-black text-white/75 transition hover:border-cyan-300/50 hover:text-white">{link.label}</a>)}</div></div>;
}
