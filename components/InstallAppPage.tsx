  ...
  return (
    ...
    >
      <section className="mx-auto w-full max-w-3xl rounded-[2rem] border border-violet-300/20 bg-white/[0.045] p-6 shadow-[0_24px_80px_rgba(76,29,149,.22)] sm:p-10">
        ...
        <h1 className="mt-4 text-balance text-4xl font-black leading-tight sm:text-5xl">
          {strings.pageTitle}
        </h1>
        ...
        <div
          data-testid="install-app-status"
          className="mt-8 rounded-3xl border border-white/10 bg-black/25 p-5 sm:p-6"
        >
          {installed ? (
            <>
              <h2 className="text-2xl font-black text-emerald-200">
                {strings.installedTitle}
              </h2>
              <p className="mt-3 leading-7 text-white/70">
                {strings.installedDescription}
              </p>
            </>
          ) : context === "custom-tab" ? (
            <div data-testid="install-custom-tab-instructions">
              <h2 className="text-2xl font-black text-yellow-200">
                {strings.customTabTitle}
              </h2>
              <p className="mt-3 leading-7 text-white/70">
                {strings.customTabDescription}
              </p>
              <a
                href={chromeIntent}
                className="mt-5 inline-flex min-h-12 items-center rounded-full bg-violet-600 px-6 py-3 font-black"
                data-testid="install-open-chrome"
              >
                {strings.openChrome}
              </a>
            </div>
            ...
            <>
              ...
              <button
                ...
                className="mt-5 min-h-12 rounded-full bg-violet-600 px-7 py-3 font-black"
                data-testid="install-app-action"
              >
                {strings.installButton}
              </button>
              {installState.outcome === "dismissed" && (
                <p className="mt-3 text-sm text-yellow-100" role="status">
                  {strings.dismissed}
                </p>
              )}
            </>
        ...
        <Link
          ...
          className="mt-7 inline-flex min-h-11 items-center font-black text-violet-200 underline underline-offset-4"
          data-testid="install-app-back-home"
        >
          {strings.backHome}
        </Link>
      </section>
