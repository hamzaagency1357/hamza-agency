...
|---|---|---:|---|---|---|---|---|---|---|---|
| SEC-01 | Authenticated Admin permission audit | Critical | Yes | `lib/server/adminMutationBoundary.ts`, Admin entity mutation API, PR116 Admin OIDC gateway | `admin_permissions` mutation can inherit deputy fallback at app/gateway layers even though DB DML is stricter | PR-A | Pending targeted super/deputy negative tests | Pending | Current source + Production DB contract | CONFIRMED |
| SEC-02 | Public support security audit | Critical | Yes | `/api/support-request`, `pr4_create_support_request` | Direct `anon`/`authenticated` EXECUTE bypasses API anti-abuse layer | PR-A | Pending direct-RPC deny + API path tests | Pending | Production catalog: SECURITY DEFINER; anon/auth EXECUTE=true | CONFIRMED |
| SEC-03 | Public support abuse controls | High | Yes | Support API / trusted backend / DB function | Direct bypass currently avoids fingerprint/rate guard; payload bounds exist but trusted-path backstop is incomplete | PR-A | Pending validation/rate-path tests | Pending | Current API and function definitions | CONFIRMED |
| SEC-04 | SECURITY DEFINER grants audit | High | Yes | Public functions / grants | 13 SECURITY DEFINER functions are anon-executable; broad revocation could break intended contracts | PR-A matrix + targeted changes only | Pending privilege assertions | N/A | Production catalog read-only evidence | CONFIRMED |
...
PR-A is intentionally limited to security-boundary fixes. No Production schema/data/auth write is authorized by this ledger. A migration may be prepared and validated in code, but applying it to Production is a separate controlled action after merge/approval.
