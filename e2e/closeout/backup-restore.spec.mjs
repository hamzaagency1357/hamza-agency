...
import { adminAction, annotations, fixture, rest, token } from "./real-runtime-helper.mjs";
...
test("real private backup persists checksum, dry run, authorization denial, and limited restore", async ({ request }, testInfo) => {
  ...
  await adminAction(request, client, "pr116_admin_backup_create", { args: { p_scope: scope, p_mode: "manual", p_notes: "denied" } }, 403);

  const created = await adminAction(request, admin, "pr116_admin_backup_create", {
    args: {
      p_scope: scope,
      p_mode: "manual",
  ...
  const dryRun = await adminAction(request, admin, "pr116_admin_backup_dry_run", { args: { p_backup: created.payload, p_scope: scope } });
  ...
  await adminAction(request, admin, "pr116_admin_backup_dry_run", { args: { p_backup: invalid, p_scope: scope } }, 503);
  await adminAction(request, client, "pr116_admin_backup_restore", { args: { p_backup: created.payload, p_scope: scope } }, 403);

  const restored = await adminAction(request, admin, "pr116_admin_backup_restore", { args: { p_backup: created.payload, p_scope: scope } });
