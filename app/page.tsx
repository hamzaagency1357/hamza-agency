...
const BRAND_PROGRAM_NAMES = new Set([
  "tiktok",
  "bigo-live",
  "yaahlan",
  "xena",
...
type ProgramCardTranslationField =
  (typeof PROGRAM_CARD_TRANSLATION_FIELDS)[number];
...
const fallbackPrograms: Program[] = [
  {
    ...
    slug: "tiktok",
    short_description:
      "برنامج مناسب لصناع المحتوى الراغبين بالنمو على TikTok.",
    description: "",
    status: "active",
    sort_order: 1,
    is_visible: true,
  ...
  },
  {
    ...
    slug: "bigo-live",
    short_description:
      "فرص بث مباشر ودعم لصناع المحتوى على BIGO LIVE.",
    description: "",
    status: "active",
    sort_order: 2,
    is_visible: true,
  ...
  },
  {
    ...
    slug: "yaahlan",
    short_description:
      "برنامج بث وتواصل اجتماعي لصناع المحتوى النشطين.",
    description: "",
    status: "active",
    sort_order: 3,
    is_visible: true,
  ...
  },
  {
    ...
    slug: "xena",
    short_description:
      "فرص محتوى وتطوير للحسابات المناسبة للبرنامج.",
    description: "",
    status: "active",
    sort_order: 4,
    is_visible: true,
  ...
  },
  {
    ...
    slug: "catchii",
    short_description:
      "برنامج متاح لصناع المحتوى ضمن وكالة حمزة.",
    description: "",
    status: "active",
    sort_order: 5,
    is_visible: true,
...
function isBrandProgram(program: Program) {
  return (
    BRAND_PROGRAM_NAMES.has(normalizeProgramKey(program.slug)) ||
    BRAND_PROGRAM_NAMES.has(normalizeProgramKey(program.name))
  );
}
...
function getRequiredFields(
  ...
  return CMS_TRANSLATION_FIELDS.filter((field) =>
...
function findHomeSection(sections: HomeSection[], sectionKey: string) {
  return (
    sections.find((section) => section.section_key === sectionKey) || null
  );
}
...
export default function HomePage() {
  ...
  const [programCardTranslations, setProgramCardTranslations] =
    useState<PublishedTranslationMap<ProgramCardTranslationField>>({});
  ...
  useEffect(() => {
    ...
    async function loadProgramTranslations() {
      ...
      if (isCurrent) {
        setProgramCardTranslations(translations);
      }
    }
  ...
  const loadPublicCmsData = useCallback(async () => {
    ...
    const [
      ...
      supabase
        .from("pages")
        .select(
          "id, title, slug, content, seo_description, is_homepage, is_published"
        )
        .eq("is_homepage", true)
    ...
    if (!announcementsResult.error && announcementsResult.data) {
      setAnnouncements(
        announcementsResult.data.filter((item) => isAnnouncementVisible(item))
      );
    }
  ...
  function normalizeAnnouncementPosition(
    value: string
  ...
  const footerLegalLinksFromSettings = useMemo(() => {
    const rawValue = settings.find(
      (item) => item.setting_key === "public_footer_links_json"
    )?.setting_value;
    ...
    try {
      ...
      const safeLinks = normalized.footerLinks.filter(
  ...
  const headerLinksFromSettings = useMemo(() => {
    const rawValue = settings.find(
      (item) => item.setting_key === "public_header_links_json"
    )?.setting_value;
    ...
    try {
      ...
      const safeLinks = normalized.headerLinks.filter(
        (link) => !link.href.startsWith("/admin")
      );
  ...
  const ctaLinksFromSettings = useMemo(() => {
    const rawValue = settings.find(
      (item) => item.setting_key === "public_cta_links_json"
    )?.setting_value;
    ...
    try {
      ...
      const safeLinks = normalized.ctaLinks.filter(
        (link) => !link.href.startsWith("/admin")
      );

      return safeLinks.length
        ? safeLinks
        : defaultPublicNavigationConfig.ctaLinks;
    } catch {
      return defaultPublicNavigationConfig.ctaLinks;
    }
  }, [settings]);
  ...
  const programsSection = findHomeSection(
    homeSections,
    "available-programs"
  );
  ...
  const pageTitle =
    homepageContent?.title?.trim() || publicSettings.heroTitle;

  const pageContent =
    homepageContent?.content?.trim() || publicSettings.heroDescription;

  const heroBadge =
    heroSection?.subtitle?.trim() || publicSettings.heroBadge;

  const heroHighlight =
    heroSection?.title?.trim() || publicSettings.heroHighlight;

  const programsTitle =
    programsSection?.title?.trim() || "البرامج المتاحة حالياً";

  const programsDescription =
    programsSection?.subtitle?.trim() ||
    programsSection?.content?.trim() ||
    "اختر البرنامج المناسب لك لعرض التفاصيل الكاملة، الشروط، نظام العمل، وما تقدمه وكالة حمزة.";

  const joinLabel =
    joinCtaSection?.title?.trim() ||
  ...
  const joinModalTitle =
    joinCtaSection?.subtitle?.trim() || "طلب الانضمام للوكالة";
    ...
    () => [
      ...
      createCmsTranslationSource({
        ...
        values: {
          ...
          summary: programsSection?.subtitle,
          content: programsSection?.content,
        },
        fallback: {
          title: programsTitle,
          summary: programsDescription,
          content: programsSection?.content || "",
        },
      }),
      ...
      createCmsTranslationSource({
        ...
        values: {
          title: joinCtaSection?.title,
          summary: joinCtaSection?.subtitle,
          content: joinCtaSection?.content,
        },
        fallback: {
          title: joinLabel,
          summary: joinModalTitle,
          content: joinCtaSection?.content || "",
        },
      }),
    ],
    ...
    () =>
      activePrograms.map((program) => {
        ...
        const hasTranslation =
          ...
          hasCompletePublishedTranslation(
            translation,
            PROGRAM_CARD_TRANSLATION_FIELDS
          );
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!form.fullName || !form.country || !form.whatsapp || !form.platform) {
      setMessage(homeCopy.requiredFieldsMessage);
      return;
    }

    if (!isSupabaseConfigured || !supabase) {
      setMessage(homeCopy.databaseUnavailableMessage);
      return;
    }
    ...
    if (localStorage.getItem(duplicateKey)) {
      setMessage(homeCopy.duplicateApplicationMessage);
      return;
    }
    ...
    if (error) {
      ...
      setMessage(homeCopy.submitErrorMessage);
      return;
    }
  ...
  return (
    <CmsPublishedTranslationsProvider sources={translationSources}>
      ...
      >
        ...
        {showSplash && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 transition-opacity duration-500">
            ...
            <div className="relative text-center">
              ...
              <p
                className="mt-5 text-sm font-black uppercase tracking-[0.35em] text-purple-100/75"
                dir="ltr"
              >
                HAMZA AGENCY
              </p>
        ...
        {publicSettings.announcementPosition === "top" &&
          activeAnnouncement && (
            <PublishedAnnouncementBar
              ...
              >
                <div className="mb-3 inline-flex rounded-full border border-green-400/30 bg-green-500/10 px-3 py-1 text-xs font-bold text-green-200">
                  {program.status === "limited"
                    ? homeCopy.statusLimited
                    : program.status === "paused"
                      ? homeCopy.statusPaused
                      : homeCopy.statusAvailable}
                </div>

                <div
                  className="text-2xl font-black"
                  dir={program.usesTranslatedName ? "ltr" : undefined}
                >
        ...
        <section className="relative z-20 mx-auto max-w-6xl px-5 pb-24">
          <div className="rounded-[2rem] border border-purple-400/20 bg-white/[0.04] p-8 backdrop-blur">
            <h2 className="text-3xl font-black">{homeCopy.whyTitle}</h2>

            <div className="mt-8 grid gap-5 md:grid-cols-3">
              {homeCopy.whyItems.map((item) => (
                <div
                  key={item}
            ...
            <div className="mx-auto my-8 max-w-3xl rounded-[2rem] border border-purple-400/25 bg-[#100014] p-6 shadow-[0_0_80px_rgba(168,85,247,0.25)]">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <input
                  ...
                  placeholder={homeCopy.fullNamePlaceholder}
                ...
                <input
                  ...
                  placeholder={homeCopy.countryPlaceholder}
                  className="w-full rounded-3xl border border-white/10 bg-black/30 p-5 text-xl outline-none focus:border-purple-400"
                />
                ...
                <input
                  ...
                  placeholder={homeCopy.whatsappPlaceholder}
                  className="w-full rounded-3xl border border-white/10 bg-black/30 p-5 text-xl outline-none focus:border-purple-400"
                />
                ...
                <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
                  <h3 className="mb-3 text-2xl font-black">
                    {homeCopy.previousExperienceTitle}
                  </h3>

                  <p className="mb-4 text-lg text-purple-200">
                    {homeCopy.previousExperiencePrompt}
                  </p>
                  ...
                  <textarea
                    ...
                    placeholder={homeCopy.previousExperiencePlaceholder}
                ...
                <textarea
                  ...
                  placeholder={homeCopy.notesPlaceholder}
          ...
          <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-4">
            <div>
              ...
              <p className="mt-5 leading-8 text-white/55">
                {publicSettings.footerDescription}
              </p>
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {publicSettings.socialLinks.map((link) => (
                    <a
                      ...
                      href={link.href}
                      target="_blank"
                      className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm text-white/70 transition hover:border-purple-400/50 hover:text-white"
                    >
                      {link.label}
                    </a>
                  ))}
            ...
            <div>
              <h3 className="font-black text-white">
                {getStaticCopy(language, "footerSiteLinks")}
              </h3>
            ...
            <div>
              <h3 className="font-black text-white">
                {getStaticCopy(language, "footerLegalPages")}
              </h3>
            ...
            <div>
              <h3 className="font-black text-white">
                {getStaticCopy(language, "footerContact")}
              </h3>
  ...
  return (
    <style>{`
      ...
      @keyframes premiumSilkOne {
        ...
        50% {
          transform: translate3d(3%, 18px, 0) rotate(-8deg);
          opacity: 0.92;
        }
      }
      ...
      @keyframes premiumGoldThread {
        ...
        50% {
          transform: translate3d(2%, 7px, 0) rotate(-4.5deg);
          opacity: 0.7;
        }
      }
