...
export default function HomePage() {
  ...
  const [programCardTranslations, setProgramCardTranslations] = useState<
    PublishedTranslationMap<ProgramCardTranslationField>
  >({});
  ...
  useEffect(() => {
    ...
    async function loadProgramTranslations() {
      const translations = await readPublishedTranslations({
        sourceType: "programs",
        language,
  ...
  async function loadPublicCmsData() {
    ...
    const [settingsResult, mediaResult, announcementsResult, programsResult, pagesResult] =
      await Promise.all([
        ...
        supabase
          .from("pages")
          .select("id, title, slug, content, seo_description, is_homepage, is_published")
          .eq("is_homepage", true)
    ...
    const { data: sections, error: sectionsError } = await supabase
      .from("sections")
      .select("id, section_key, title, subtitle, content, is_visible")
      .eq("page_id", homePage.id)
      .eq("is_visible", true)
      .in("section_key", ["hero", "available-programs", "join-cta"])
      .order("sort_order", { ascending: true });
  ...
  const translationSources = useMemo<CmsPublishedTranslationSource[]>(
    () => [
      createCmsTranslationSource({
        sourceKey: "home-page",
        sourceType: "pages",
        sourceId: homepageContent?.id,
        values: {
          title: homepageContent?.title,
          summary: homepageContent?.seo_description,
          content: homepageContent?.content,
        },
        fallback: { title: pageTitle, content: pageContent },
      }),
      createCmsTranslationSource({
        sourceKey: "home-hero",
        sourceType: "sections",
        sourceId: heroSection?.id,
        values: { title: heroSection?.title, summary: heroSection?.subtitle, content: heroSection?.content },
        fallback: { title: heroHighlight, summary: heroBadge, content: heroSection?.content || "" },
      }),
      createCmsTranslationSource({
        sourceKey: "home-available-programs",
        sourceType: "sections",
        sourceId: programsSection?.id,
        values: { title: programsSection?.title, summary: programsSection?.subtitle, content: programsSection?.content },
        fallback: { title: programsTitle, summary: programsDescription, content: programsSection?.content || "" },
      }),
      createCmsTranslationSource({
        sourceKey: "home-join-cta",
        sourceType: "sections",
        sourceId: joinCtaSection?.id,
        values: { title: joinCtaSection?.title, summary: joinCtaSection?.subtitle, content: joinCtaSection?.content },
        fallback: { title: joinLabel, summary: joinModalTitle, content: joinCtaSection?.content || "" },
      }),
    ],
      ...
      activePrograms.map((program) => {
        ...
        return {
          ...
          displaySummary: hasTranslation
            ? translation?.summary || program.short_description || program.description || "عرض تفاصيل البرنامج"
      ...
      >
        ...
        {showSplash && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 transition-opacity duration-500">
            ...
            <div className="relative text-center">
              <img src={logoUrl} alt={publicSettings.englishName} className="mx-auto h-36 w-36 rounded-3xl object-cover shadow-[0_0_90px_rgba(168,85,247,0.7)]" />
              <p className="mt-5 text-sm font-black uppercase tracking-[0.35em] text-purple-100/75" dir="ltr">HAMZA AGENCY</p>
            </div>
          </div>
        )}
        ...
        <nav className="relative z-20 mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-6">
          <Link href="/" className="flex items-center gap-3">
            <img src={logoUrl} alt={publicSettings.englishName} className="h-12 w-12 rounded-xl object-cover shadow-[0_0_25px_rgba(168,85,247,0.45)]" />
            <div>
          ...
          <div className="hidden items-center gap-2 lg:flex">
            {headerLinksFromSettings.map((link) => <Link key={link.href} href={link.href} className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-bold text-white/70 backdrop-blur transition hover:border-purple-400/50 hover:bg-purple-500/10 hover:text-white">{getSharedLabel(link.href, link.label)}</Link>)}
          </div>
        </nav>

        <div className="relative z-20 mx-auto mb-4 max-w-7xl px-5 lg:hidden">
          <div className="flex gap-2 overflow-x-auto pb-2">
            {headerLinksFromSettings.map((link) => <Link key={link.href} href={link.href} className="shrink-0 rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-sm font-bold text-white/75 backdrop-blur">{getSharedLabel(link.href, link.label)}</Link>)}
          </div>
        </div>
        ...
        <section className="relative z-20 mx-auto max-w-7xl px-5 pb-20 pt-10 text-center">
          <HeroVideoVisual />
          {publicSettings.announcementPosition === "inside_hero" && activeAnnouncement && <div className="relative z-20 mx-auto mb-8 max-w-4xl"><AnnouncementBar announcement={activeAnnouncement} animation={publicSettings.announcementAnimation} speed={publicSettings.announcementSpeed} rounded /></div>}
          <div className="relative z-20">
            <img src={logoUrl} alt={`${publicSettings.englishName} Logo`} className="mx-auto mb-8 h-44 w-44 rounded-[2rem] object-cover shadow-[0_0_95px_rgba(168,85,247,0.65)]" />
            <div className="mx-auto mb-6 inline-flex rounded-full border border-purple-400/30 bg-purple-500/10 px-5 py-2 text-sm text-purple-100 backdrop-blur">
              <CmsPublishedText sourceKey="home-hero" field="summary" fallback={heroBadge} />
            </div>
            <h1 className="text-5xl font-black leading-tight md:text-7xl">
              <CmsPublishedText sourceKey="home-page" field="title" fallback={pageTitle} />
              <span className="block bg-gradient-to-r from-purple-300 via-white to-yellow-300 bg-clip-text text-transparent"><CmsPublishedText sourceKey="home-hero" field="title" fallback={heroHighlight} /></span>
            </h1>
            <p className="mx-auto mt-6 max-w-3xl text-lg leading-9 text-white/80 md:text-2xl"><CmsPublishedText sourceKey="home-page" field="content" fallback={pageContent} /></p>
            <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
              {primaryJoinCta.isVisible !== false && <button onClick={() => setShowJoinForm(true)} className="rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 px-9 py-4 text-lg font-bold shadow-[0_0_40px_rgba(168,85,247,0.45)] transition hover:scale-[1.03]"><CmsPublishedText sourceKey="home-join-cta" field="title" fallback={joinLabel} /></button>}
              {viewProgramsCta.isVisible !== false && <Link href={viewProgramsCta.href} className="rounded-full border border-white/15 bg-white/[0.05] px-9 py-4 text-lg font-bold text-white/80 backdrop-blur transition hover:border-purple-400/50 hover:bg-purple-500/10">{getSharedLabel(viewProgramsCta.href, viewProgramsCta.label)}</Link>}
            </div>
          </div>
        </section>

        <section className="relative z-20 mx-auto grid max-w-6xl grid-cols-2 gap-5 px-5 pb-20 lg:grid-cols-4">
          {homeStats.map(([number, label]) => <div key={label} className="rounded-3xl border border-white/10 bg-white/[0.05] p-7 text-center backdrop-blur"><div className="bg-gradient-to-r from-purple-300 to-yellow-300 bg-clip-text text-4xl font-black text-transparent">{number}</div><div className="mt-3 text-white/75">{label}</div></div>)}
        </section>

        <section className="relative z-20 mx-auto max-w-7xl px-5 pb-24">
          <h2 className="text-center text-4xl font-black"><CmsPublishedText sourceKey="home-available-programs" field="title" fallback={programsTitle} /></h2>
          <p className="mx-auto mt-4 max-w-2xl text-center text-white/60"><CmsPublishedText sourceKey="home-available-programs" field="summary" fallback={programsDescription} /></p>
          <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-5">
            {translatedPrograms.map((program) => <Link key={program.id} href={program.slug ? `/programs/${program.slug}` : "/programs"} className="rounded-3xl border border-white/10 bg-white/[0.04] p-6 text-center backdrop-blur transition hover:border-purple-400/60 hover:bg-purple-500/10"><div className="mb-3 inline-flex rounded-full border border-green-400/30 bg-green-500/10 px-3 py-1 text-xs font-bold text-green-200">{program.status === "limited" ? "قبول محدود" : program.status === "paused" ? "متوقف مؤقتاً" : "متاح الآن"}</div><div className="text-2xl font-black" dir={program.usesTranslatedName ? "ltr" : undefined}>{program.translatedName}</div><div className="mt-3 text-sm leading-7 text-white/60" dir={language === "ar" ? "rtl" : "ltr"}>{program.displaySummary}</div></Link>)}
          </div>
          {viewProgramsCta.isVisible !== false && <div className="mt-10 text-center"><Link href={viewProgramsCta.href} className="inline-flex rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 px-8 py-4 font-black shadow-[0_0_35px_rgba(168,85,247,0.22)]">{getSharedLabel(viewProgramsCta.href, viewProgramsCta.label)}</Link></div>}
        </section>

        <section className="relative z-20 mx-auto max-w-6xl px-5 pb-24"><div className="rounded-[2rem] border border-purple-400/20 bg-white/[0.04] p-8 backdrop-blur"><h2 className="text-3xl font-black">لماذا وكالة حمزة؟</h2><div className="mt-8 grid gap-5 md:grid-cols-3">{whyUsItems.map((item) => <div key={item} className="rounded-2xl border border-white/10 bg-black/25 p-5">{item}</div>)}</div><div className="mt-8 grid gap-4 md:grid-cols-3"><Link href="/about" className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-center font-bold text-white/75 transition hover:border-purple-400/50">{getSharedLabel("/about", "تعرف علينا")}</Link><Link href="/services" className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-center font-bold text-white/75 transition hover:border-purple-400/50">{getSharedLabel("/services", "خدمات الوكالة")}</Link><Link href="/contact" className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-center font-bold text-white/75 transition hover:border-purple-400/50">{getSharedLabel("/contact", "تواصل معنا")}</Link></div></div></section>

        {showJoinForm && <div className="fixed inset-0 z-40 overflow-y-auto bg-black/80 p-4 backdrop-blur"><div className="mx-auto my-8 max-w-3xl rounded-[2rem] border border-purple-400/25 bg-[#100014] p-6 shadow-[0_0_80px_rgba(168,85,247,0.25)]"><div className="mb-6 flex items-center justify-between"><button onClick={() => { setMessage(""); setShowJoinForm(false); }} className="rounded-full border border-white/15 px-5 py-2 text-white/70">إغلاق</button><h2 className="text-3xl font-black"><CmsPublishedText sourceKey="home-join-cta" field="summary" fallback={joinModalTitle} /></h2></div><form onSubmit={handleSubmit} className="space-y-5"><input value={form.fullName} onChange={(event) => updateField("fullName", event.target.value)} placeholder="الاسم الثلاثي" className="w-full rounded-3xl border border-white/10 bg-black/30 p-5 text-xl outline-none focus:border-purple-400" /><input value={form.country} onChange={(event) => updateField("country", event.target.value)} placeholder="الدولة" className="w-full rounded-3xl border border-white/10 bg-black/30 p-5 text-xl outline-none focus:border-purple-400" /><input value={form.whatsapp} onChange={(event) => updateField("whatsapp", event.target.value)} placeholder="رقم واتساب" className="w-full rounded-3xl border border-white/10 bg-black/30 p-5 text-xl outline-none focus:border-purple-400" /><select value={form.platform} onChange={(event) => updateField("platform", event.target.value)} className="w-full rounded-3xl border border-white/10 bg-black/30 p-5 text-xl outline-none focus:border-purple-400">{activePrograms.map((program) => <option key={program.id} value={program.name || ""}>{program.name}</option>)}</select><div className="rounded-3xl border border-white/10 bg-black/30 p-5"><h3 className="mb-3 text-2xl font-black">خبرات سابقة</h3><p className="mb-4 text-lg text-purple-200">هل عملت على برامج أو وكالات أخرى سابقاً؟</p><textarea value={form.previousExperience} onChange={(event) => updateField("previousExperience", event.target.value)} placeholder="اكتب خبراتك السابقة إن وجدت" className="min-h-40 w-full resize-none bg-transparent text-xl outline-none" /></div><textarea value={form.notes} onChange={(event) => updateField("notes", event.target.value)} placeholder="ملاحظات إضافية" className="min-h-36 w-full resize-none rounded-3xl border border-white/10 bg-black/30 p-5 text-xl outline-none focus:border-purple-400" />{message && <div className="rounded-3xl border border-yellow-500/40 bg-yellow-500/10 p-5 text-center text-xl font-bold text-yellow-100">{message}</div>}<button type="submit" disabled={isSubmitting} className="w-full rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 px-8 py-5 text-2xl font-black disabled:opacity-60">{isSubmitting ? "جارٍ الإرسال..." : "إرسال الطلب"}</button></form></div></div>}
