...
export default function HomePage() {
  ...
  const publicSettings = useMemo(() => {
    const agencyName = getSetting(
      ["agency_name", "site_name", "brand_name"],
      "وكالة حمزة"
    );

    const englishName = getSetting(
      ["site_name_en", "english_name", "company_name"],
      "HAMZA AGENCY"
    );
    ...
    return {
      ...
      heroDescription: getSetting(
        ["home_hero_description", "hero_description"],
  ...
  const homeBackgroundMedia = getMediaByPurpose({
    pageSlug: "home",
    category: "background",
  });
  ...
  const hasGeneratedHomeBackground =
    homeBackgroundMedia?.file_url?.startsWith("generated://") || true;
  ...
  return (
    ...
    >
      ...
      {hasGeneratedHomeBackground && (
        <>
          <div className="fixed left-8 top-20 -z-10 h-72 w-72 animate-pulse rounded-full bg-purple-700/20 blur-3xl" />
          <div className="fixed bottom-16 right-8 -z-10 h-72 w-72 animate-pulse rounded-full bg-yellow-500/10 blur-3xl" />
          <div className="fixed inset-0 -z-10 opacity-20 [background-image:radial-gradient(circle_at_1px_1px,rgba(255,255,255,0.25)_1px,transparent_0)] [background-size:42px_42px]" />
        </>
      )}

      {announcements.length > 0 && (
        <section className="border-b border-yellow-400/20 bg-yellow-400/10 px-4 py-3 text-center">
          <div className="mx-auto max-w-7xl text-sm font-bold text-yellow-100 md:text-base">
            <span className="text-yellow-200">{announcements[0].title}</span>
