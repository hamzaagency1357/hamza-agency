...
const fallbackJobs: Job[] = [
  ...
  },
  {
    ...
    requirements:
      "معرفة جيدة بمنصات البث، قدرة على التنظيم، وخبرة سابقة بالوكالات أو البرامج تعتبر ميزة.",
    status: "open",
    sort_order: 2,
    is_visible: true,
  },
  {
    ...
    requirements:
      "دقة في جمع المعلومات، متابعة جيدة، والتزام بعدم تنفيذ أي خدمة قبل تأكيد الإدارة.",
    status: "open",
    sort_order: 3,
    is_visible: true,
  },
...
export default function JobsPage() {
  ...
  async function loadJobs() {
    ...
    const { data, error } = await supabase
      .from("jobs")
      .select(
        "id, title, slug, department, location, job_type, short_description, description, requirements, status, sort_order, is_visible"
      )
      .eq("is_visible", true)
      .eq("status", "open")
  ...
  const openJobs = useMemo(() => {
    return jobs.filter((job) => {
      return job.is_visible !== false && (job.status === "open" || !job.status);
    });
  }, [jobs]);

  function updateForm(key: keyof JobApplicationForm, value: string) {
    setForm((current) => ({
      ...current,
      [key]: value,
    }));
    ...
    >
      ...
      <section className="relative z-10 mx-auto max-w-7xl">
        <nav className="mb-8 flex items-center justify-between gap-4">
          <Link
            href="/"
            className="rounded-full border border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-bold text-white/75 backdrop-blur"
          >
            العودة للرئيسية
          </Link>
          ...
          <Link
            href="/contact"
            className="rounded-full border border-yellow-400/20 bg-yellow-400/10 px-5 py-3 text-sm font-bold text-yellow-100 backdrop-blur"
          >
            تواصل معنا
          </Link>
        </nav>
          ...
          {openJobs.length === 0 ? (
            ...
            <div className="grid gap-6 lg:grid-cols-3">
              {openJobs.map((job, index) => (
                <article
                  key={`${job.id || "fallback"}-${job.slug || index}`}
                ...
                >
                  ...
                  <h3 className="text-3xl font-black">
                    {job.title || "وظيفة متاحة"}
                  </h3>

                  <p className="mt-4 leading-8 text-white/66">
                    {job.short_description ||
                      job.description ||
                      "فرصة عمل ضمن وكالة حمزة."}
                  </p>
                  ...
                  <button
                    ...
                    className="mt-6 w-full rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 px-6 py-4 font-black text-white shadow-[0_0_32px_rgba(168,85,247,0.24)]"
                  >
                    تقديم على الوظيفة
                  </button>
                </article>
...
function ApplyModal({
  ...
  updateForm: (key: keyof JobApplicationForm, value: string) => void;
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/80 p-4 backdrop-blur">
      <div className="mx-auto my-8 max-w-3xl rounded-[2rem] border border-purple-400/25 bg-[#0d0014] p-6 shadow-[0_0_80px_rgba(168,85,247,0.22)]">
        <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <button
            onClick={closeApply}
            className="rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-3 font-bold text-white/75"
          >
            إغلاق
          </button>
        </div>
