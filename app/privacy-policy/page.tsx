...
export default async function PrivacyPolicyPage() {
  ...
  const overview = getSectionContent(overviewSection, {
    ...
    content: "توضح سياسة الخصوصية آلية جمع واستخدام وحماية بيانات المتقدمين والعملاء عند استخدام موقع وكالة حمزة ونماذجها.",
  });
  const dataUsage = getSectionContent(dataUsageSection, {
    title: "استخدام البيانات",
    subtitle: "تُستخدم البيانات لمراجعة الطلبات وتحسين التواصل",
    content: "تُستخدم البيانات المقدمة عبر النماذج لأغراض مراجعة الطلبات، التواصل، تقديم الدعم، وتحسين تجربة المستخدم.",
  });
  ...
  const translationSources: CmsPublishedTranslationSource[] = [
    ...
    createCmsTranslationSource({
      ...
      values: { title: dataUsageSection?.title, summary: dataUsageSection?.subtitle, content: dataUsageSection?.content },
      fallback: { title: dataUsage.title, summary: dataUsage.subtitle, content: dataUsage.content },
    }),
  ];
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-5 py-16">
          ...
          <div className="rounded-[2rem] border border-purple-400/20 bg-black/35 p-7 shadow-[0_0_45px_rgba(168,85,247,0.12)] backdrop-blur md:p-10">
            ...
            <h1 className="text-5xl font-black leading-tight md:text-7xl">
              ...
              <span className="block bg-gradient-to-r from-purple-300 via-white to-yellow-300 bg-clip-text text-transparent"><CmsPublishedText sourceKey="privacy-overview" field="summary" fallback={overview.subtitle} /></span>
            </h1>
            <p className="mt-8 max-w-5xl text-xl leading-10 text-white/75">{hasPageContent ? <CmsPublishedText sourceKey="privacy-page" field="content" fallback={page?.content || ""} /> : <CmsPublishedText sourceKey="privacy-overview" field="content" fallback={overview.content} />}</p>
          </div>
          <div className="mt-10 rounded-[2rem] border border-yellow-400/20 bg-yellow-500/10 p-7 backdrop-blur">
            <h2 className="text-3xl font-black text-yellow-100"><CmsPublishedText sourceKey="privacy-data-usage" field="title" fallback={dataUsage.title} /></h2>
            <p className="mt-3 text-lg font-bold text-yellow-100/80"><CmsPublishedText sourceKey="privacy-data-usage" field="summary" fallback={dataUsage.subtitle} /></p>
            <p className="mt-5 leading-9 text-white/75"><CmsPublishedText sourceKey="privacy-data-usage" field="content" fallback={dataUsage.content} /></p>
          </div>
          <PrivacyPolicyCards />
          <PrivacyApplicationDataPanel />
