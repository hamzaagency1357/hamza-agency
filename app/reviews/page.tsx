...
async function getReviews() {
  ...
  const { data, error } = await supabase
    .from("reviews")
    .select(
      "id, reviewer_name, country, platform, rating, content, avatar_url, is_featured, sort_order, status, is_visible, created_at"
    )
    .eq("is_visible", true)
    .eq("status", "published")
...
function renderStars(rating: number | null) {
  ...
  return Array.from({ length: 5 }).map((_, index) => (
    <span
      key={index}
      className={index < safeRating ? "text-yellow-300" : "text-white/20"}
    >
      ★
    </span>
  ));
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-5 py-16">
        ...
        <section className="mt-14">
          <div className="mb-6">
            ...
            <p className="mt-3 max-w-3xl leading-8 text-white/60">
              تقييمات يمكن إدارتها لاحقاً من لوحة التحكم، مع إمكانية إظهارها أو
              إخفائها وترتيبها.
            </p>
          </div>
        ...
        <section className="mt-14 rounded-[2rem] border border-green-400/20 bg-green-500/10 p-7 text-center backdrop-blur">
          <h2 className="text-3xl font-black text-green-100">
            لديك تجربة مع وكالة حمزة؟
          </h2>

          <p className="mx-auto mt-4 max-w-2xl leading-8 text-white/70">
            يمكن لاحقاً إضافة نموذج تقييم رسمي من الموقع، أما حالياً يمكن إرسال
            رأيك لفريق الوكالة عبر واتساب ليتم مراجعته قبل النشر.
          </p>
    ...
    >
      <div className="flex items-start gap-4">
        {review.avatar_url ? (
          ...
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-purple-400/25 bg-purple-500/10 text-lg font-black text-purple-100">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="break-words text-xl font-black">
            {review.reviewer_name || "عميل وكالة حمزة"}
          </h3>

          <div className="mt-2 flex flex-wrap gap-2 text-xs text-white/55">
            <span>{review.country || "غير محدد"}</span>
            <span>•</span>
            <span>{review.platform || "HAMZA AGENCY"}</span>
          </div>
        </div>
      </div>

      <div className="mt-5 text-xl tracking-widest">
