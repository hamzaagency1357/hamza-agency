...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!post) return { title: siteContext.language === "ar" ? "المقال غير موجود | HAMZA AGENCY" : siteContext.language === "tr" ? "Makale bulunamadı | HAMZA AGENCY" : "Article not found | HAMZA AGENCY", robots: { index: false, follow: false } };
  ...
  const title = post.copy.seoTitle?.trim() || `${post.copy.title} | HAMZA AGENCY`;
  const description = post.copy.seoDescription?.trim() || post.copy.excerpt;
  const image = post.featuredImage || `${SITE_URL}/opengraph-image`;
  return { ...base, title, description, alternates: { canonical, languages: getLanguageAlternates(publicPath) }, openGraph: { title, description, url: canonical, siteName: "HAMZA AGENCY", locale: siteContext.language === "ar" ? "ar_AR" : siteContext.language === "tr" ? "tr_TR" : "en_US", type: "article", publishedTime: post.publishedAt || undefined, modifiedTime: post.updatedAt || post.publishedAt || undefined, tags: post.tags, images: [{ url: image, width: 1200, height: 630, alt: post.copy.title }] }, twitter: { card: "summary_large_image", title, description, images: [image] }, robots: { index: true, follow: true, googleBot: { index: true, follow: true, "max-image-preview": "large", "max-snippet": -1, "max-video-preview": -1 } } };
}
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!post) notFound();
  const [related, taxonomy] = await Promise.all([getServerRelatedBlogPosts(post, language, 3), getServerBlogTaxonomy(language)]);
  ...
  return <PublicLanguageMain className="relative min-h-screen overflow-hidden bg-[#070009] text-white">
    ...
    <main className="relative z-10 mx-auto flex min-h-screen max-w-6xl flex-col px-5 py-12 md:py-16">
      ...
      <article className="overflow-hidden rounded-[2rem] border border-purple-400/20 bg-black/35 shadow-[0_0_45px_rgba(168,85,247,0.12)] backdrop-blur">
        ...
        <div className="p-7 md:p-10"><div className="flex flex-wrap items-center gap-3 text-sm text-white/60"><span className="rounded-full border border-purple-400/20 bg-purple-500/10 px-3 py-2 font-black text-purple-100">{t.category}: {taxonomy.categories[post.category]?.label || post.category}</span>{publishedValue ? <time dateTime={publishedValue}>{t.published} {new Date(publishedValue).toLocaleDateString(locale)}</time> : null}{post.updatedAt && post.updatedAt !== publishedValue ? <time dateTime={post.updatedAt}>{t.updated} {new Date(post.updatedAt).toLocaleDateString(locale)}</time> : null}</div>
        <h1 className="mt-5 text-4xl font-black leading-tight md:text-6xl">{post.copy.title}</h1><p className="mt-5 max-w-3xl text-lg leading-9 text-white/70">{post.copy.excerpt}</p>
        ...
        <div className="prose prose-invert mt-10 max-w-none leading-9 text-white/80 prose-a:text-purple-200 prose-headings:text-white prose-li:my-2" dangerouslySetInnerHTML={{ __html: safeHtml }} /></div>
      </article>
