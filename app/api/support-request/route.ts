...
type SupportCreateResult={allowed?:boolean;code?:string;supportCode?:string;verification?:string;status?:string;retentionDays?:number};
...
export async function POST(request:NextRequest){
 ...
 try{
  const data=await callOidcGateway<SupportCreateResult>(request,"support_request_create",{identity:fingerprint(request),payload:{language,subject,context,contactType,contactValue,consent:true}});
  if(data?.allowed===false)return fail(data.code==="try_again_later"?429:400,"rejected");
