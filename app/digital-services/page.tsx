...
const digitalServices = [
  { title: "شحن المنصات", text: "استقبال طلبات الشحن وتنظيم بيانات المنصة والحساب، ثم تأكيد التفاصيل عبر القنوات الرسمية قبل أي متابعة.", tag: "Top Up" },
  { title: "سحب الأرباح", text: "مساعدة صناع المحتوى في ترتيب طلبات السحب وفهم الخطوات المطلوبة حسب نظام كل برنامج، بدون طلب كلمات مرور أو بيانات حساسة.", tag: "Withdrawals" },
  { title: "متابعة الطلبات الرقمية", text: "تسجيل الطلبات داخل لوحة الإدارة لمراجعة الحالة وتحديثها بشكل منظم، مع تقليل ضياع الرسائل أو تكرار الطلبات.", tag: "Requests" },
  { title: "دعم واتساب رسمي", text: "التواصل مع فريق الوكالة لتأكيد الطلب أو توضيح البيانات الناقصة عبر رقم الواتساب الرسمي المعتمد في الموقع.", tag: "WhatsApp" },
];
const rules = ["لا تطلب وكالة حمزة كلمة مرور حسابك أو رمز التحقق أو أي بيانات دخول حساسة.", "لا يتم تنفيذ أي خدمة قبل تأكيد التفاصيل النهائية مع العميل عبر القنوات الرسمية.", "الخدمات الرقمية تُراجع حسب نوع الطلب والمنصة وسياسات البرنامج، ولا يتم ضمان نتائج تخالف سياسات المنصات.", "إرسال الطلب من الموقع يساعد على تنظيم المتابعة داخل لوحة الإدارة وتقليل الأخطاء أو تكرار المعلومات.", "أي تفاصيل مالية أو تنفيذية يتم تأكيدها بوضوح قبل المتابعة، وليس بمجرد إرسال النموذج.", "في حال وجود نقص في البيانات، قد يتواصل فريق الوكالة عبر واتساب لاستكمال المعلومات اللازمة فقط."];
const workflow = ["اختيار الخدمة المطلوبة", "تعبئة نموذج طلب الخدمة", "مراجعة البيانات الأساسية", "تأكيد التفاصيل عبر واتساب عند الحاجة", "تحديث حالة الطلب من لوحة الإدارة"];
const trustNotes = ["استخدم رقم الواتساب الظاهر في الموقع فقط عند التواصل مع الوكالة.", "احتفظ برمز طلب الخدمة بعد الإرسال لاستخدامه في المتابعة لاحقاً.", "تأكد من كتابة معرف الحساب والمنصة بشكل صحيح لتجنب تأخير المعالجة.", "لا ترسل صور بطاقات أو معلومات شخصية غير مطلوبة داخل نموذج الطلب."];
      ...
      <main className="relative min-h-screen overflow-hidden bg-[#070009] text-white">
        ...
        <section className="relative z-10 mx-auto max-w-7xl px-5 py-16">
          ...
          <div className="rounded-[2rem] border border-purple-400/20 bg-black/35 p-7 shadow-[0_0_45px_rgba(168,85,247,0.12)] backdrop-blur md:p-10">
            ...
            <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-5 text-sm leading-7 text-white/65">ملاحظة مهمة: هذه الصفحة لتنظيم الطلبات ومتابعتها فقط. أي تنفيذ أو تفاصيل نهائية يتم تأكيدها بوضوح مع فريق الوكالة عبر القنوات الرسمية.</div>
            <div className="mt-8 flex flex-col gap-4 sm:flex-row"><Link href="/service-request" className="inline-flex justify-center rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 px-8 py-4 font-black text-white shadow-[0_0_35px_rgba(168,85,247,0.28)] transition hover:scale-[1.02]">تقديم طلب خدمة رقمية</Link><a href={`https://wa.me/${cleanWhatsapp}?text=${encodeURIComponent("مرحباً، أريد الاستفسار عن الخدمات الرقمية في وكالة حمزة.")}`} target="_blank" className="inline-flex justify-center rounded-full border border-green-400/25 bg-green-500/10 px-8 py-4 font-black text-green-100 transition hover:bg-green-500/20">تواصل واتساب</a></div>
          </div>
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">{digitalServices.map((service) => <div key={service.title} className="rounded-[2rem] border border-white/10 bg-white/[0.045] p-6 backdrop-blur"><div className="mb-5 inline-flex rounded-full border border-purple-400/20 bg-purple-500/10 px-4 py-2 text-xs font-bold text-purple-100">{service.tag}</div><h2 className="text-3xl font-black">{service.title}</h2><p className="mt-4 leading-8 text-white/68">{service.text}</p></div>)}</div>
          <div className="mt-10 rounded-[2rem] border border-yellow-400/20 bg-yellow-500/10 p-7 backdrop-blur"><h2 className="text-3xl font-black text-yellow-100">تنبيه مهم قبل الطلب</h2><div className="mt-6 grid gap-4 md:grid-cols-2">{rules.map((rule) => <div key={rule} className="rounded-2xl border border-white/10 bg-black/25 p-5 text-white/75">{rule}</div>)}</div></div>
          <div className="mt-10 rounded-[2rem] border border-purple-400/20 bg-purple-500/10 p-7 backdrop-blur"><h2 className="text-3xl font-black">طريقة طلب الخدمة</h2><div className="mt-6 grid gap-4 md:grid-cols-5">{workflow.map((step, index) => <div key={step} className="rounded-2xl border border-white/10 bg-black/25 p-5"><div className="mb-4 flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-r from-yellow-500 to-purple-600 font-black">{index + 1}</div><p className="leading-7 text-white/75">{step}</p></div>)}</div></div>
          <div className="mt-10 rounded-[2rem] border border-cyan-400/20 bg-cyan-500/10 p-7 backdrop-blur"><h2 className="text-3xl font-black text-cyan-100">إرشادات ثقة وأمان</h2><div className="mt-6 grid gap-4 md:grid-cols-2">{trustNotes.map((note) => <div key={note} className="rounded-2xl border border-white/10 bg-black/25 p-5 text-white/75">{note}</div>)}</div></div>
          <div className="mt-10 rounded-[2rem] border border-green-400/20 bg-green-500/10 p-7 text-center backdrop-blur"><h2 className="text-3xl font-black"><CmsPublishedText sourceKey="service-request-cta" field="title" fallback={requestCta.title} /></h2><p className="mx-auto mt-4 max-w-2xl leading-8 text-white/70"><CmsPublishedText sourceKey="service-request-cta" field="summary" fallback={requestCta.subtitle} /></p><p className="mx-auto mt-4 max-w-2xl leading-8 text-white/70"><CmsPublishedText sourceKey="service-request-cta" field="content" fallback={requestCta.content} /></p><div className="mt-7 flex flex-col justify-center gap-4 sm:flex-row"><Link href="/service-request" className="inline-flex justify-center rounded-full bg-gradient-to-r from-purple-600 to-fuchsia-600 px-8 py-4 font-black text-white shadow-2xl">تقديم طلب خدمة رقمية</Link><a href={`https://wa.me/${cleanWhatsapp}?text=${encodeURIComponent("مرحباً، أريد الاستفسار عن الخدمات الرقمية في وكالة حمزة.")}`} target="_blank" className="inline-flex justify-center rounded-full bg-green-500 px-8 py-4 font-black text-white shadow-2xl">تواصل واتساب</a></div></div>
        </section>
      </main>
    </CmsPublishedTranslationsProvider>
  );
