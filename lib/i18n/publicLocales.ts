...
const nonIndexablePublicPaths = new Set([
  "/application-status",
  "/service-status",
  "/track",
  "/blog/rss",
...
export function splitLocalizedPathname(
  pathname: string
): { language: SiteLanguage; publicPath: string } {
...
export function isDynamicCmsPublicPath(pathname: string) {
  ...
  return (
    dynamicCmsPathPattern.test(publicPath) &&
    !publicRouteSet.has(publicPath) &&
    !reservedCmsSegments.has(firstSegment) &&
    !blogArticlePathPattern.test(publicPath) &&
    !programPathPattern.test(publicPath)
  );
}
...
export function isSupportedPublicPath(pathname: string) {
  ...
  return (
    publicRouteSet.has(publicPath) ||
    programPathPattern.test(publicPath) ||
    isBlogPublicPath(publicPath) ||
    isDynamicCmsPublicPath(publicPath)
  );
}
...
export function getBlogSlugFromPath(pathname: string) {
  const match = stripLocalePrefix(pathname).match(
    /^\/blog\/([a-z0-9]+(?:-[a-z0-9]+)*)$/
  );
  return match?.[1] || null;
...
export function isIndexablePublicPath(pathname: string) {
  ...
  return (
    ...
    !nonIndexablePublicPaths.has(publicPath)
  );
}
