...
export function splitLocalizedPathname(pathname: string): { language: SiteLanguage; publicPath: string } {
  const normalized = normalizePublicPathname(pathname);
  const match = normalized.match(localizedPrefixPattern);
  if (!match || !isSiteLanguage(match[1])) return { language: "ar", publicPath: normalized };
...
export function localizePublicPath(pathname: string, language: SiteLanguage) {
  const url = new URL(pathname, SITE_URL);
  const publicPath = stripLocalePrefix(url.pathname);
  const localized = language === "ar" ? publicPath : publicPath === "/" ? `/${language}` : `/${language}${publicPath}`;
  return `${localized}${url.search}${url.hash}`;
}

export function localizePublicHref(href: string, language: SiteLanguage) {
  if (!href.startsWith("/") || href.startsWith("//") || href.startsWith("/api/") || href.startsWith("/admin")) return href;
  return localizePublicPath(href, language);
}
...
export function getLanguageAlternates(pathname: string) {
  ...
  return { ar: getLocalizedAbsoluteUrl(publicPath, "ar"), en: getLocalizedAbsoluteUrl(publicPath, "en"), tr: getLocalizedAbsoluteUrl(publicPath, "tr"), "x-default": getLocalizedAbsoluteUrl(publicPath, "ar") };
}
export function isDynamicCmsPublicPath(pathname: string) {
  const publicPath = stripLocalePrefix(pathname);
  const firstSegment = publicPath.split("/").filter(Boolean)[0] || "";
  return dynamicCmsPathPattern.test(publicPath) && !publicRouteSet.has(publicPath) && !reservedCmsSegments.has(firstSegment) && !blogArticlePathPattern.test(publicPath) && !programPathPattern.test(publicPath);
}
export function isBlogPublicPath(pathname: string) { const publicPath = stripLocalePrefix(pathname); return publicPath === "/blog" || publicPath === "/blog/rss" || blogArticlePathPattern.test(publicPath); }
export function isSupportedPublicPath(pathname: string) { const publicPath = stripLocalePrefix(pathname); return publicRouteSet.has(publicPath) || programPathPattern.test(publicPath) || isBlogPublicPath(publicPath) || isDynamicCmsPublicPath(publicPath); }
...
export function getBlogSlugFromPath(pathname: string) { const match = stripLocalePrefix(pathname).match(/^\/blog\/([a-z0-9]+(?:-[a-z0-9]+)*)$/); return match?.[1] || null; }
