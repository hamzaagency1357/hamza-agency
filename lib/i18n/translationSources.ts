...
export type TranslationSourceDefinition = {
  sourceType: TranslationSourceType;
  label: string;
  table: "programs" | "pages" | "sections" | "faqs" | "knowledge_base" | "partners" | "jobs" | "reviews";
...
export const TRANSLATION_SOURCE_DEFINITIONS: readonly TranslationSourceDefinition[] = [
  ...
  { sourceType: "pages", label: "صفحات CMS", table: "pages", titleKeys: ["title"], summaryKeys: ["seo_description", "summary"], contentKeys: ["content"] },
  { sourceType: "sections", label: "أقسام CMS", table: "sections", titleKeys: ["title"], summaryKeys: ["subtitle"], contentKeys: ["content"] },
  { sourceType: "faqs", label: "الأسئلة الشائعة", table: "faqs", titleKeys: ["question", "title"], summaryKeys: ["category"], contentKeys: ["answer", "content"] },
  { sourceType: "knowledge_base", label: "مركز المعرفة", table: "knowledge_base", titleKeys: ["title"], summaryKeys: ["summary", "category"], contentKeys: ["content", "answer", "body"] },
  { sourceType: "partners", label: "الشركاء", table: "partners", titleKeys: ["name", "title"], summaryKeys: ["category", "type"], contentKeys: ["description", "summary"] },
  { sourceType: "jobs", label: "الوظائف", table: "jobs", titleKeys: ["title"], summaryKeys: ["short_description"], contentKeys: ["description"], departmentKeys: ["department"], locationKeys: ["location"], jobTypeKeys: ["job_type"], requirementsKeys: ["requirements"] },
...
export function isTranslationFieldName(value: unknown): value is TranslationFieldName {
  return typeof value === "string" && [...BASE_TRANSLATION_FIELDS, ...PROGRAM_DETAIL_TRANSLATION_FIELDS, ...JOB_DETAIL_TRANSLATION_FIELDS, ...REVIEW_DETAIL_TRANSLATION_FIELDS].includes(value as TranslationFieldName);
}
...
export function getTranslationFieldNamesForSource(sourceType: TranslationSourceType): TranslationFieldName[] {
  if (sourceType === "programs") return [...BASE_TRANSLATION_FIELDS, ...PROGRAM_DETAIL_TRANSLATION_FIELDS];
  if (sourceType === "jobs") return [...BASE_TRANSLATION_FIELDS, ...JOB_DETAIL_TRANSLATION_FIELDS];
  if (sourceType === "reviews") return [...BASE_TRANSLATION_FIELDS, ...REVIEW_DETAIL_TRANSLATION_FIELDS];
...
export function toTranslationSourceItem(sourceType: TranslationSourceType, sourceId: string, row: Record<string, unknown>): TranslationSourceItem | null {
  const definition = getTranslationSourceDefinition(sourceType);
  if (!definition || !sourceId.trim()) return null;
  const item: TranslationSourceItem = { sourceType, sourceId: sourceId.trim(), title: getTranslationFieldText(row, definition.titleKeys), summary: getTranslationFieldText(row, definition.summaryKeys), content: getTranslationFieldText(row, definition.contentKeys) };
  ...
  if (sourceType === "reviews") item.country = getTranslationFieldText(row, definition.countryKeys || []);
  const hasTranslatableText = getTranslationFieldNamesForSource(sourceType).some((field) => Boolean(item[field]?.trim()));
