...
-- Final PR116 least-privilege boundary.
-- Public submissions must enter through Vercel OIDC -> service-role gateway only.
revoke execute on function public.pr99_submit_application(jsonb,text,timestamptz,text) from public, anon, authenticated;
revoke execute on function public.pr99_submit_service_request(jsonb,text,timestamptz,text) from public, anon, authenticated;
revoke execute on function public.pr99_submit_contact(jsonb,text,timestamptz,text) from public, anon, authenticated;
revoke execute on function public.pr99_submit_ai_support(jsonb,text,timestamptz,text) from public, anon, authenticated;
revoke execute on function public.pr99_submit_job_application(jsonb,text,timestamptz,text) from public, anon, authenticated;
grant execute on function public.pr99_submit_application(jsonb,text,timestamptz,text) to service_role;
grant execute on function public.pr99_submit_service_request(jsonb,text,timestamptz,text) to service_role;
grant execute on function public.pr99_submit_contact(jsonb,text,timestamptz,text) to service_role;
...
revoke execute on function public.lookup_public_agency_application(text,text) from public, anon, authenticated;
revoke execute on function public.lookup_public_service_request(text) from public, anon, authenticated;
grant execute on function public.lookup_public_agency_application(text,text) to service_role;
grant execute on function public.lookup_public_service_request(text) to service_role;

-- Defense in depth: even after a valid Vercel OIDC assertion, Preview may not
-- dispatch production-sensitive writes into the shared Production database.
...
begin
  ...
  if p_oidc_environment='preview' and p_action in (
    'application_submit','service_request_submit','job_application_submit','contact_submit','ai_support_submit','review_submit'
  ) then
...
alter function public.pr100_oidc_gateway(text,bigint,text,text,text,text,text,text,text,text,text,text,bigint,bigint) owner to postgres;
...
-- Regression guard against EXECUTE privilege drift.
do $pr116_final_security_contract$
declare
  fn oid;
begin
  foreach fn in array array[
    ...
    if fn is null then raise exception 'pr116_security_boundary_function_missing'; end if;
    if has_function_privilege('anon',fn,'EXECUTE') or has_function_privilege('authenticated',fn,'EXECUTE') then
      raise exception 'pr116_security_boundary_execute_exposed: %', fn::regprocedure;
    end if;
    if not has_function_privilege('service_role',fn,'EXECUTE') then
      raise exception 'pr116_security_boundary_service_role_missing: %', fn::regprocedure;
    end if;
  end loop;
end
