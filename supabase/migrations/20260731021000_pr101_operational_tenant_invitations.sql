...
-- Additive only. This migration was not previously applied and deletes no production data.
begin;
...
-- Official model: exactly one mutable membership per user in each tenant.
...
create index if not exists tenant_invitations_email_idx on public.tenant_invitations(tenant_id,email,created_at desc);
create unique index if not exists tenant_invitations_one_pending_uidx
  on public.tenant_invitations(tenant_id,email,role,coalesce(program_id,0::bigint)) where status='invited';
...
begin
  ...
  for v_key,v_value in select key,value from jsonb_each(p_permissions) loop
    if not (v_key=any(v_allowed)) or jsonb_typeof(v_value)<>'boolean' then
      raise exception 'permission_not_allowed';
    end if;
  end loop;
...
begin
  ...
  if p_action in ('create','resend') and not public.current_user_has_tenant_role(p_tenant_id,array['super_admin','tenant_admin']) then
...
-- Tenant-scoped expiration is the only authenticated expiration entry point.
...
begin
  if p_tenant_id is null or not public.current_user_has_tenant_role(p_tenant_id,array['super_admin','tenant_admin']) then
    raise exception 'forbidden' using errcode='42501';
  end if;
...
-- Global scheduler-only expiration is private and unavailable through the exposed Data API schema.
...
as $$
declare
  v_actor uuid:=auth.uid(); v_email text:=lower(btrim(coalesce(p_email,'')));
  v_invitation public.tenant_invitations%rowtype; v_permissions jsonb;
begin
  if v_actor is null or p_tenant_id is null then raise exception 'forbidden' using errcode='42501'; end if;
  if not public.current_user_has_tenant_role(p_tenant_id,array['super_admin','tenant_admin']) then raise exception 'forbidden' using errcode='42501'; end if;
  if p_role not in ('creator','client','employee','partner','tenant_admin') then raise exception 'invalid_role'; end if;
  if p_role='tenant_admin' and not public.current_user_has_tenant_role(p_tenant_id,array['super_admin']) then raise exception 'super_admin_required' using errcode='42501'; end if;
  if v_email !~ '^[^@[:space:]]+@[^@[:space:]]+\.[^@[:space:]]+$' or length(v_email)>254 then raise exception 'invalid_invitation'; end if;
  if p_token_hash !~ '^[a-f0-9]{64}$' then raise exception 'invalid_invitation'; end if;
  if p_expires_at<=now() or p_expires_at>now()+interval '30 days' then raise exception 'invalid_invitation'; end if;
  v_permissions:=private.normalize_invitation_permissions(p_role,p_permissions);
  if p_program_id is not null and not exists(select 1 from public.programs p where p.id=p_program_id and p.tenant_id=p_tenant_id) then raise exception 'invalid_invitation'; end if;
...
as $$
declare
  v_user uuid:=auth.uid(); v_email text:=lower(coalesce(auth.jwt()->>'email',''));
  v_invitation public.tenant_invitations%rowtype; v_membership public.tenant_memberships%rowtype;
begin
  if v_user is null or v_email='' or p_expected_tenant_id is null then raise exception 'invitation_rejected' using errcode='42501'; end if;
  if p_token_hash !~ '^[a-f0-9]{64}$' then raise exception 'invitation_rejected'; end if;
  ...
  if not found or v_invitation.status<>'invited' then raise exception 'invitation_rejected'; end if;
  if v_invitation.expires_at<=now() then
    update public.tenant_invitations set status='expired',updated_at=now() where id=v_invitation.id;
    raise exception 'invitation_rejected';
  end if;
  if lower(v_invitation.email)<>v_email then raise exception 'invitation_rejected' using errcode='42501'; end if;
  ...
  on conflict(tenant_id,user_id) do update set role=excluded.role,status='active',program_id=excluded.program_id,
    permissions=excluded.permissions,mfa_required=excluded.mfa_required,updated_at=now()
  ...
  if not found then raise exception 'invitation_rejected'; end if;
  insert into public.tenant_admin_audit(tenant_id,actor_id,action,entity_type,entity_id,after_data)
...
as $$
declare
  v_actor uuid:=auth.uid(); v_before public.tenant_memberships%rowtype; v_after public.tenant_memberships%rowtype;
  v_permissions jsonb; v_super_count integer;
begin
  if v_actor is null or p_tenant_id is null or not public.current_user_has_tenant_role(p_tenant_id,array['super_admin','tenant_admin']) then raise exception 'forbidden' using errcode='42501'; end if;
  ...
  if v_before.role='super_admin' then
    select count(*) into v_super_count from public.tenant_memberships where tenant_id=p_tenant_id and role='super_admin' and status='active';
    if v_super_count<=1 then raise exception 'last_super_admin_protected' using errcode='42501'; end if;
  ...
  v_permissions:=private.normalize_invitation_permissions(p_role,p_permissions);
  update public.tenant_memberships set status=p_status,role=p_role,program_id=p_program_id,permissions=v_permissions,
    mfa_required=(p_role='tenant_admin'),updated_at=now() where id=p_membership_id and tenant_id=p_tenant_id returning * into v_after;
  ...
  values(p_tenant_id,'تم تحديث العضوية','تم تحديث حالة أو صلاحيات عضويتك.','tenant_membership',v_after.user_id,
    'tenant_membership_updated:'||v_after.id||':'||extract(epoch from v_after.updated_at)::bigint,
    jsonb_build_object('status',v_after.status,'role',v_after.role),
    'tenant.membership.updated:'||v_after.id||':'||extract(epoch from v_after.updated_at)::bigint,
    'tenant.membership.updated','tenant_membership',v_after.id::text,'created','high') on conflict do nothing;
  return v_after;
end;
$$;
...
comment on table public.tenant_invitations is 'Stores only SHA-256 invitation token hashes. Raw tokens are generated server-side, shown once and never persisted.';
comment on function public.expire_tenant_invitations(uuid) is 'Authenticated tenant-scoped expiration; NULL and cross-tenant writes are rejected.';
comment on function private.expire_all_tenant_invitations() is 'Server-only global scheduler function unavailable to authenticated users.';
comment on function public.accept_tenant_invitation(uuid,text) is 'Single-use, host-tenant-bound and email-bound invitation acceptance; reactivates one existing suspended/revoked tenant membership.';
