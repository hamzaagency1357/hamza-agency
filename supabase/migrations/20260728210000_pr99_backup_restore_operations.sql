...
returns jsonb language plpgsql security definer set search_path=pg_catalog,public as $$
declare v_actor text:=public.pr99_require_admin();v_row jsonb;v_columns text;v_updates text;v_count integer:=0;v_id_column text:='id';
begin
 ...
 select string_agg(format('%I',column_name),',' order by ordinal_position),string_agg(format('%1$I=excluded.%1$I',column_name),',' order by ordinal_position)
 into v_columns,v_updates from information_schema.columns where table_schema='public' and table_name=p_table and is_generated='NEVER' and column_name<>'id';
 if v_columns is null then raise exception 'Restore schema unavailable';end if;
 for v_row in select value from jsonb_array_elements(p_rows) loop
  if not(v_row?'id') then raise exception 'Restore row has no id';end if;
  execute format('insert into public.%1$I select * from jsonb_populate_record(null::public.%1$I,$1) on conflict (%2$I) do update set %3$s',p_table,v_id_column,v_updates) using v_row;
  v_count:=v_count+1;
 end loop;
...
begin
 ...
 update public.restore_operations set status='completed',summary=jsonb_build_object('results',v_results,'pre_restore_backup',v_pre->>'backup_code'),completed_at=now() where id=v_operation;
...
exception when others then
 if v_operation is not null then update public.restore_operations set status='failed',summary=jsonb_build_object('safe_error',sqlstate),completed_at=now() where id=v_operation;end if;
 insert into public.notifications(title,message,type,is_read,recipient_role,event_key,event_type,entity_type,occurred_at,priority,metadata) values('فشل الاستعادة','توقفت عملية الاستعادة بأمان ولم يكتمل الالتزام.','restore_failure',false,'admin','restore-failure:'||extract(epoch from clock_timestamp())::bigint,'failed','restore_operation',now(),'critical',jsonb_build_object('safe_error',sqlstate)) on conflict(event_key) where event_key is not null do nothing;
 raise;
end $$;
...
begin
 ...
 execute format('update public.%I set is_visible=false where id::text=$1',p_table) using p_id;
...
exception when undefined_column then
 execute format('update public.%I set status=''archived'' where id::text=$1',p_table) using p_id;
 return jsonb_build_object('trash_id',v_trash,'was_published',v_published);
end $$;
...
begin
 ...
 select * into v_item from public.trash_items where id=p_trash_id for update;if not found then raise exception 'Trash item unavailable';end if;
 execute format('delete from public.%I where id::text=$1',v_item.item_type) using v_item.item_id;
 update public.trash_items set restore_status='permanently_deleted',item_data=null,data='{}'::jsonb where id=p_trash_id;
...
-- Private scheduled backup: stored only in the RLS-protected backups table.
