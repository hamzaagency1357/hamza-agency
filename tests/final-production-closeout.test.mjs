...
test("install page remains gated by a real browser install opportunity",()=>{const source=read("components/InstallAppPage.tsx");const normalized=compact(source);assert.match(normalized,/ready=context==="browser"&&installState\.available&&!installed/);assert.ok(source.includes("hamza:pwa-install-request"));assert.ok(source.includes('data-testid="install-app-action"'));assert.ok(source.includes('data-testid="install-app-fallback"'));assert.ok(normalized.includes("onClick={requestInstall}"));assert.ok(source.includes("getPublicIdentity"))});
