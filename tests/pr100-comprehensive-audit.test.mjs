...
test("tracking routes are supported but excluded from indexing and sitemap output", async () => {
  ...
  assert.match(locales, /nonIndexablePublicPaths\s*=\s*new Set\(\[[^\]]*"\/track"/);
  assert.match(sitemap, /PUBLIC_ROUTE_PATHS\.filter\(\(path\)\s*=>\s*isIndexablePublicPath\(path\)\)/);
  assert.match(sitemap, /isIndexablePublicPath/);
...
test("application PII is protected by action permissions and program-scoped RLS", async () => {
  const migrationPaths = ["supabase/migrations/20260730185634_pr100_comprehensive_audit_hardening.sql","supabase/migrations/20260730185730_pr100_comprehensive_audit_hardening.sql","supabase/migrations/20260730185735_pr100_comprehensive_audit_hardening.sql"];
  ...
  assert.match(migration, /current_admin_has_module_permission\('applications', 'can_view'\)/);
  assert.match(migration, /current_admin_has_module_permission\('applications', 'can_edit'\)/);
  assert.match(migration, /admin_user\.role = 'program_admin'/);
  assert.match(migration, /regexp_replace\(lower\(coalesce\(agency_applications\.platform, ''\)/);
});
