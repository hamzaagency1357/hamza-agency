...
test("cookie settings use normal pages and preserve the consent contract", () => {
  ...
  for (const path of ["app/cookie-settings/page.tsx","app/en/cookie-settings/page.tsx","app/tr/cookie-settings/page.tsx"]) assert.ok(fs.existsSync(path), `${path} must exist`);
  assert.match(consent, /localizePublicHref\(\s*"\/cookie-settings"\s*,\s*locale\s*\)/);
  assert.match(footer, /localizePublicHref\(\s*"\/cookie-settings"\s*,\s*language\s*\)/);
...
test("Install App localization is URL-owned without optimistic language selection", () => {
  ...
  for (const path of ["app/install-app/page.tsx","app/en/install-app/page.tsx","app/tr/install-app/page.tsx"]) assert.doesNotMatch(read(path), /language="(?:ar|en|tr)"/);
